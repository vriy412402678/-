    ...
    $.fn.extend({
        takungaeImgup : function(opt, serverCallBack) {
            if (typeof opt != "object") {
                layer.msg('参数错误!',{time: 2000,icon:5});
                return;
            }
            ...
            var defaults = {
                fileType : [ "jpg", "png", "bmp", "jpeg","JPG","PNG","JPEG","BMP",'swf'], // 上传文件的类型 "jpg", "png", "bmp", "jpeg","JPG","PNG","JPEG","BMP"
            ...
            $(this).change(function() {
                ...
                if (fileList.length > imageNum|| totalNum > imageNum) {
                    layer.msg("上传图片数目不可以超过5个，请重新选择",{time: 2000,icon:5}); // 一次选择上传超过5个
                    ...
                    for ( var i = 0; i < fileList.length; i++) {
                        ...
                        var domain = 'http://'+document.domain+'/public/frontend/images/a7.png';
                        $img0.attr("src",domain).appendTo($section);
                    ...
                    if (newStr.split(".")[0] != null) {
                        ...
                        if (jQuery.inArray(type, defaults.fileType) > -1) {
                            ...
                            if (file.size >= defaults.fileSize) {
                                layer.msg('文件大小"' + file.name + '"超出10M限制！',{time: 2000,icon:5});
                            } else {
                                arrFiles.push(file);
                            }
                        } else {
                            layer.msg('您上传的"' + file.name + '"不符合上传类型',{time: 2000,icon:5});
                        }
                    } else {
                        layer.msg('您上传的"' + file.name + '"无法识别类型',{time: 2000,icon:5});
                    }
                }
                return arrFiles;
            }
